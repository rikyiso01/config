		...
		settings = {
			basedpyright = {
				analysis = {
					...
					stubPath = "/home/riky/backup/Documents/Projects/Python/common-stubs",
					extraPaths = { "typings" },
				},
			},
		},
...
vim.lsp.config(
	"rust_analyzer",
	{ capabilities = lsp_capabilities, cmd = { "rust-analyzer" }, settings = {
...
vim.lsp.config(
	...
	{
		capabilities = lsp_capabilities,
		settings = { ltex = { language = "auto" }, additionalRules = { languageModel = "/home/riky/.ngrams" } },
	}
)
...
require("formatter").setup({
	filetype = {
		...
		haskell = {
			function()
				return { exe = "fourmolu", args = { "--no-cabal", "-" }, stdin = true }
			end,
		},
		java = {
			function()
		...
		javascript = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.js" }, stdin = true }
			end,
		},
		typescript = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.ts" }, stdin = true }
			end,
		},
		typescriptreact = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.tsx" }, stdin = true }
			end,
		},
		json = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.json" }, stdin = true }
			end,
		},
		jsonc = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.jsonc" }, stdin = true }
			end,
		},
		yaml = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.yml" }, stdin = true }
			end,
		},
		markdown = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.md" }, stdin = true }
			end,
		},
		html = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.html" }, stdin = true }
			end,
		},
		htmldjango = {
			function()
		...
		css = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.css" }, stdin = true }
			end,
		},
		graphql = {
			function()
				return { exe = "prettier", args = { "--stdin-filepath=test.graphql" }, stdin = true }
			end,
		},
		xml = {
			function()
				return { exe = "tidy", args = { "-i", "-xml" }, stdin = true }
			end,
		},
		nix = {
			function()
		...
		dockerfile = {
			function()
				return { exe = "dockerfmt", stdin = true }
			end,
		},
		toml = {
			function()
		...
		dart = {
			function()
				return { exe = "dart", args = { "format" }, stdin = false }
			end,
		},
		dhall = {
			function()
				return { exe = "dhall", args = { "format" }, stdin = true }
			end,
		},
		just = {
			function()
				return { exe = "just", args = { "--dump" }, stdin = true }
			end,
		},
		ruby = {
			function()
				return { exe = "rufo", args = { "--simple-exit" }, stdin = true }
			end,
		},
		cs = {
			function()
				return { exe = "dotnet-csharpier", stdin = true }
			end,
		},
		...
		erlang = {
			function()
				return { exe = "erlfmt", args = { "-" }, stdin = true }
			end,
		},
		lua = {
			function()
...
require("trouble").setup({
	...
	preview = { type = "main", size = { width = 0.8 } },
})
...
dap.configurations.cs = {
	{
		...
		processId = function()
			local cmd =
				"netstat -nlp | grep 127.0.0.1:$(jq '.profiles | values[] | .applicationUrl // \"http://localhost:5000\"' Properties/launchSettings.json | sed -rn 's/.*http:\\/\\/localhost:([0-9]+).*/\\1/p' | head -n 1) | sed -rn 's/.* ([0-9]+)\\/Reply.Ferrar.*/\\1/p' | head -n 1"
