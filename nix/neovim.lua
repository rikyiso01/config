...
dap.configurations.cs = {
  {
    ...
    request = "attach",
    processId = function()
        local cmd="netstat -nlp | grep 127.0.0.1:$(jq '.profiles | values[] | .applicationUrl // \"http://localhost:5000\"' Properties/launchSettings.json | sed -rn 's/.*http:\\/\\/localhost:([0-9]+).*/\\1/p' | head -n 1) | sed -rn 's/.* ([0-9]+)\\/Reply.Ferrar.*/\\1/p' | head -n 1"
