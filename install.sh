...
sudo bash -c "echo $USER ALL=(ALL) NOPASSWD:$HOME/.nix-profile/bin/tlp, $HOME/.nix-profile/bin/dockerd -H unix\:///var/run/docker.sock, $HOME/.nix-profile/bin/systemctl start cups, $HOME/.nix-profile/bin/systemctl start avahi-daemon, $HOME/.nix-profile/bin/thermald --no-daemon --systemd, $HOME/.nix-profile/bin/auto-cpufreq --daemon" >>/etc/sudoers
