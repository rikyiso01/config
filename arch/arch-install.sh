...
echo "options rd.luks.name=$(blkid "${part}${name}2" -o value -s UUID)=root rd.luks.options=discard,no-read-workqueue,no-write-workqueue root=/dev/mapper/root splash quiet rw" >> /mnt/boot/loader/entries/arch.conf
